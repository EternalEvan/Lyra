...
class SpatialVidFramePackLightningModel(pl.LightningModule):
    def __init__(
        ...
        if resume_ckpt_path is not None:
            ...
            # 🔧 过滤掉cam_encoder相关的权重，使其保持随机初始化
            filtered_state_dict = {}
            for key, value in state_dict.items():
                if 'cam_encoder' not in key:
                    filtered_state_dict[key] = value
                else:
                    print(f"🔧 跳过加载cam_encoder权重: {key}")
            
            # 使用strict=False允许部分权重不匹配
            missing_keys, unexpected_keys = self.pipe.dit.load_state_dict(filtered_state_dict, strict=False)
            print(f'✅ 加载checkpoint: {resume_ckpt_path}')
            print(f'🔧 cam_encoder保持随机初始化，未加载预训练权重')
            if missing_keys:
                print(f'⚠️  Missing keys (预期的，包含cam_encoder): {len(missing_keys)} keys')
            if unexpected_keys:
    ...
    def on_save_checkpoint(self, checkpoint):
        checkpoint_dir = "/share_zhuyixuan05/zhuyixuan05/ICLR2026/spatialvid/spatialvid_framepack_random"
