...
class DynamicSekaiDataset(torch.utils.data.Dataset):
    ...
    def select_dynamic_segment_framepack(self, full_latents):
        ...
        # 🔧 修改：1x帧改为起始4帧 + 最后1帧
        # 起始4帧（不足的用0填充）
        clean_latent_start_indices = torch.arange(start_frame_compressed, min(start_frame_compressed + 4, condition_end_compressed))
        clean_latent_1x_indices = torch.tensor([condition_end_compressed - 1])
        clean_latent_indices = torch.cat([clean_latent_start_indices, clean_latent_1x_indices])
        ...
        if condition_frames_compressed >= 2:
            ...
            clean_latent_2x_indices = torch.arange(clean_latent_2x_start, condition_end_compressed)
        else:
        ...
        if condition_frames_compressed >= 1:
            ...
            clean_latent_4x_indices = torch.arange(clean_4x_start, condition_end_compressed)
        else:
    ...
    def prepare_framepack_inputs(self, full_latents, segment_info):
        """🔧 准备FramePack风格的多尺度输入 - 修正版，支持起始4帧+最后1帧"""
        ...
        # 🔧 1x条件帧：起始4帧 + 最后1帧，固定长度为5帧
        clean_latent_indices = segment_info['clean_latent_indices']
        
        # 创建固定长度5的latents，初始化为0
        clean_latents = torch.zeros(B, C, 5, H, W, dtype=full_latents.dtype)
        clean_latent_indices_final = torch.full((5,), -1, dtype=torch.long)  # -1表示padding
        
        # 填充真实的clean latents
        if len(clean_latent_indices) > 0:
            # 获取真实的latent数据
            actual_clean_latents = full_latents[:, :, clean_latent_indices, :, :]
            actual_frames = actual_clean_latents.shape[2]
            
            if actual_frames <= 5:
                # 如果实际帧数不超过5，从前往后填充
                clean_latents[:, :, :actual_frames, :, :] = actual_clean_latents
                clean_latent_indices_final[:actual_frames] = clean_latent_indices
            else:
                # 如果超过5帧，取前4帧+最后1帧
                clean_latents[:, :, :4, :, :] = actual_clean_latents[:, :, :4, :, :]
                clean_latents[:, :, 4:5, :, :] = actual_clean_latents[:, :, -1:, :, :]
                clean_latent_indices_final[:4] = clean_latent_indices[:4]
                clean_latent_indices_final[4:5] = clean_latent_indices[-1:]
        ...
        return {
            ...
            'latent_indices': segment_info['latent_indices'],
            'clean_latent_indices': clean_latent_indices_final,  # 🔧 使用真实索引（含-1填充）
    ...
    def create_pose_embeddings(self, cam_data, segment_info):
        """🔧 创建pose embeddings - 为所有帧（condition + target）提取camera信息，支持起始4帧+最后1帧"""
        ...
        if 'wan_video_dit' in model_names:
            ...
            for name, cls in zip(model_names, model_classes):
                if name == 'wan_video_dit':
                    ...
                    new_model_classes.append(WanModelFuture4)  # 替换为新的类
...
class DynamicLightningModelForTrain(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        wandb.log({
            ...
            "dataset_name": dataset_name,  # 🔧 新增：记录数据集名称
            "scene_name": scene_name,     # 🔧 新增：记录场景名称
    ...
    def on_save_checkpoint(self, checkpoint):
        checkpoint_dir = "/share_zhuyixuan05/zhuyixuan05/ICLR2026/sekai/sekai_walking_framepack_4"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--resume_ckpt_path", type=str, default="/share_zhuyixuan05/zhuyixuan05/ICLR2026/sekai/sekai_walking_framepack_4/step34290_framepack.ckpt")
