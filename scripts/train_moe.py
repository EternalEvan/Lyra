...
class MultiDatasetDynamicDataset(torch.utils.data.Dataset):
        ...
        for config in self.dataset_configs:
            ...
            for dataset_path in dataset_paths:
                ...
                if os.path.exists(dataset_path):                    
                    ...
                    elif dataset_type in ['sekai', 'spatialvid', 'openx']:
    ...
    def create_nuscenes_pose_embeddings_framepack(self, scene_info, segment_info):
        """创建NuScenes风格的pose embeddings - 每帧都相对上一帧（7维）"""
        keyframe_poses = scene_info['keyframe_poses']
        # 生成所有需要的关键帧索引
        start_frame = segment_info['start_frame']
        total_frames = segment_info['condition_frames'] + segment_info['target_frames']
        keyframe_indices = []
        for i in range(total_frames + 1):  # +1是因为需要前后两帧
            idx = (start_frame + i) * self.time_compression_ratio
    ...
    def select_dynamic_segment(self, full_latents, dataset_type, scene_info=None):
        ...
        for compressed_idx in range(start_frame_compressed, target_end_compressed):
            ...
            elif dataset_type == 'openx' or 'sekai' or "nuscenes":  # 🔧 新增openx处理
...
class MultiDatasetLightningModelForTrain(pl.LightningModule):
    ...
    def add_moe_components(self):
        """🔧 添加MoE相关组件 - 简化版，只为每个block添加MoE，全局processor在WanModelMoe中"""
        ...
        # 为每个block添加MoE组件（modality processors已经在WanModelMoe中全局创建）
        dim = self.pipe.dit.blocks[0].self_attn.q.weight.shape[0]
        ...
        self.pipe.dit.openx_processor = ModalityProcessor("openx", 13, unified_dim)  # OpenX使用13维输入，类似sekai但独立处理
    ...
    def on_save_checkpoint(self, checkpoint):
        checkpoint_dir = "/share_zhuyixuan05/zhuyixuan05/ICLR2026/framepack_moe"
        os.makedirs(checkpoint_dir, exist_ok=True)
        
        current_step = self.global_step
        checkpoint.clear()
        t = time.strftime("%Y%m%d-%H%M%S")   # 20250923-153047

        
        state_dict = self.pipe.denoising_model().state_dict()
        torch.save(state_dict, os.path.join(checkpoint_dir, f"step{current_step}_nus_moe_from_{t}.ckpt"))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--resume_ckpt_path", type=str, default="/home/zhuyixuan05/ReCamMaster/nus_dynamic/step15000_dynamic.ckpt")
    ...
    parser.add_argument("--moe_loss_weight", type=float, default=0.1, help="MoE损失权重")
